...
const openCascadeHelper = {
  ...
  joinPrimitives(facelist) {
    ...
    facelist.forEach(myface => {
      ...
      for (let x = 0; x < myface.tri_indexes.length / 3; x++) {
        ...
        obTR++;
		face_ids.push(myface.face_id);
      }

      advance = obP;
    });
    return [face_ids, locVertexcoord, locNormalcoord, locTriIndices];
  },
  ...
  generateGeometry(face_ids, tot_triangle_count, locVertexcoord, locNormalcoord, locTriIndices) {
    const vertices = [];
    const faces = [];
    function v(x, y, z) {
      vertices.push(new Vector3(x, y, z));
    }
    function f3(face_id, a, b, c, n1_x, n1_y, n1_z, n2_x, n2_y, n2_z, n3_x, n3_y, n3_z) {
      const geometry = new THREE.BufferGeometry();
	  //geometry.setAttribute('position', 
      //                      new THREE.BufferAttribute( new Float32Array( [
      //                      -5, 0, 0, 5, 0, 0,  0, 5, 0  ], 3 ) ) );
      geometry.setAttribute('tangent', 
                            new THREE.BufferAttribute(new Float32Array([n1_x, n1_y, n1_z, n2_x, n2_y, n2_z, n3_x, n3_y, n3_z]), 3 ));
	  
      // specify triangles, as triplets of indexes into the vertex list.
	  geometry.setIndex( new THREE.BufferAttribute( [ a, b, c ], 1 ) );
	  geometry.setAttribute('face_id', new THREE.BufferAttribute(new Uint32Array([face_id]), 1 ));
	  faces.push(geometry);
	  
      //faces.push(new BufferGeometry(a, b, c, [
